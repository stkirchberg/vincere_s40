...
public class CryptoEngine {
    ...
    private int rotr(int x, int n) { return (x >>> n) | (x << (32 - n)); }
    private int s0(int x) { return rotr(x, 2) ^ rotr(x, 13) ^ rotr(x, 22); }
    private int s1(int x) { return rotr(x, 6) ^ rotr(x, 11) ^ rotr(x, 25); }
